(() => {
  ...
  const wait=(fn,timeout=7000)=>new Promise((resolve,reject)=>{
  ...
  function installCard(){
    ...
    card.innerHTML=`<div class="dss42-title">DSS v4.3 · RESULTAT FINAL REAL</div><div class="dss42-sub">Genera el VECTOR MASTER, aplica la recepta premium i exporta exactament el mateix SVG que veus a la previsualització. L'exportació antiga ja no s'utilitza per al fitxer final.</div><button id="dss42Generate" class="btn gold dss42-btn">GENERAR SAMARRETA FINAL</button><div id="dss42Status" class="dss42-status">Preparat.</div>`;
  ...
  function audit(svg){
    ...
    const final=svg.getAttribute('data-dss-final')||'';
    return {ok:pieces===8&&premium>=6,pieces,premium,uses,final,msg:`${pieces}/8 peces · ${premium} grups premium · ${uses} objectes <use>`};
  }
  async function generateFinal(){
    const btn=byId('dss42Generate');if(btn)btn.disabled=true;
    try{
      setStatus('1/5 · Preparant briefing…','warn');goStep(2);await new Promise(r=>setTimeout(r,160));
      let gen=findButton('Generar VECTOR MASTER');if(!gen){await wait(()=>findButton('Generar VECTOR MASTER'),3500);gen=findButton('Generar VECTOR MASTER')}
      if(!gen)throw new Error('No trobo el motor VECTOR MASTER');
      setStatus('2/5 · Generant les 8 peces i objectes vectorials…','warn');gen.click();goStep(3);
      await wait(()=>activeSvg(),6000);await new Promise(r=>setTimeout(r,650));
      ...
      const a=audit(svg);
      if(!a.ok)throw new Error('El master visual no ha passat el gate intern: '+a.msg);
      setStatus('3/5 · Master premium verificat: '+a.msg+'.','ok');
  ...
  function serializedSvg(){
    ...
    const clone=svg.cloneNode(true);clone.setAttribute('xmlns','http://www.w3.org/2000/svg');clone.setAttribute('data-dss-production',VERSION);clone.setAttribute('data-dss-export-source','live-preview');
  ...
  function exportExactSvg(){
    try{const out=serializedSvg();downloadBlob(new Blob([out],{type:'image/svg+xml'}),`DSS_FINAL_${slug(currentDesign())}.svg`);setStatus('4/5 · SVG FINAL descarregat des del master visible actual.','ok')}catch(e){setStatus('Error exportant SVG: '+e.message,'bad')}
  }
  async function exportExactPng(){
    try{
      const out=serializedSvg(),blob=new Blob([out],{type:'image/svg+xml'}),url=URL.createObjectURL(blob),img=new Image();
      await new Promise((res,rej)=>{img.onload=res;img.onerror=rej;img.src=url});
      const maxW=4096,w=maxW,h=Math.round(maxW*9809/17812),c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d');ctx.fillStyle='#111';ctx.fillRect(0,0,w,h);ctx.drawImage(img,0,0,w,h);URL.revokeObjectURL(url);
      const png=await new Promise(res=>c.toBlob(res,'image/png',0.95));downloadBlob(png,`DSS_PREVIEW_${slug(currentDesign())}.png`);setStatus('4/5 · PREVIEW PNG generat exactament des del master visible.','ok');
    }catch(e){setStatus('Error exportant PREVIEW: '+(e.message||e),'bad')}
  }
  ...
  function installFinalDownloads(){
    const s3=byId('s3');if(!s3)return;let card=byId('dss42Downloads');if(card)card.remove();card=document.createElement('div');card.id='dss42Downloads';card.className='dss42-final';
    const a=audit(activeSvg());
    card.innerHTML=`<div class="dss42-title">Exportació final v4.3</div><div class="dss42-sub">Aquests botons exporten el master visible actual, incloses les capes premium i els objectes vectorials. No reutilitzen l'exportador antic.</div><div class="dss43-audit">Gate intern: <b>${a.ok?'OK':'ERROR'}</b> · ${a.msg}</div><div class="dss42-downloads"><button id="dss42Svg" class="btn gold">Descarregar SVG FINAL</button><button id="dss43Png" class="btn">Descarregar PREVIEW EXACTE</button></div><div class="dss42-downloads"><button id="dss42Zip" class="btn">Descarregar VECTOR PACKAGE.zip</button><button id="dss42QA" class="btn">Anar a QA / Fabricant</button></div>`;
  ...
  function chatLinkAutostart(){if(!location.hash.includes('dss='))return;setTimeout(()=>byId('dss42Generate')?.click(),750)}
  function boot(){style();installCard();const brand=document.querySelector('.brand small');if(brand)brand.textContent='v4.3 · FINAL MASTER = PREVIEW = EXPORT';const uploadCard=byId('artFile')?.closest('.card');if(uploadCard){const h=uploadCard.querySelector('h2');if(h)h.textContent='Artwork extern (opcional / compatibilitat)'}chatLinkAutostart();console.info('DSS v4.3 Final exact export loaded')}
